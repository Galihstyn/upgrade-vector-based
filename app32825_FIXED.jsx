...
const getWarpMetrics = (el) => {
  ...
  const positions = chars.map((char, charIdx) => {
    ...
    // Bolt Optimization: Hoist trigonometric calculations and inline corner
    // evaluations to avoid intermediate array allocations in this hot loop
    const cosRad = Math.cos(rad);
    const sinRad = Math.sin(rad);

    const x1 = dx - halfW * cosRad + halfH * sinRad;
    const y1 = dy - halfW * sinRad - halfH * cosRad;
    if (x1 < minX) minX = x1;
    if (x1 > maxX) maxX = x1;
    if (y1 < minY) minY = y1;
    if (y1 > maxY) maxY = y1;

    const x2 = dx + halfW * cosRad + halfH * sinRad;
    const y2 = dy + halfW * sinRad - halfH * cosRad;
    if (x2 < minX) minX = x2;
    if (x2 > maxX) maxX = x2;
    if (y2 < minY) minY = y2;
    if (y2 > maxY) maxY = y2;

    const x3 = dx + halfW * cosRad - halfH * sinRad;
    const y3 = dy + halfW * sinRad + halfH * cosRad;
    if (x3 < minX) minX = x3;
    if (x3 > maxX) maxX = x3;
    if (y3 < minY) minY = y3;
    if (y3 > maxY) maxY = y3;

    const x4 = dx - halfW * cosRad - halfH * sinRad;
    const y4 = dy - halfW * sinRad + halfH * cosRad;
    if (x4 < minX) minX = x4;
    if (x4 > maxX) maxX = x4;
    if (y4 < minY) minY = y4;
    if (y4 > maxY) maxY = y4;
