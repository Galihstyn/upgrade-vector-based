...
const getWarpMetrics = (el) => {
  ...
  const positions = chars.map((char, charIdx) => {
    ...
    const wCos = halfW * cosRad;
    const wSin = halfW * sinRad;
    const hCos = halfH * cosRad;
    const hSin = halfH * sinRad;

    // Corner 1: -halfW, -halfH
    const x1 = dx - wCos + hSin;
    const y1 = dy - wSin - hCos;
    // Corner 2: halfW, -halfH
    const x2 = dx + wCos + hSin;
    const y2 = dy + wSin - hCos;
    // Corner 3: halfW, halfH
    const x3 = dx + wCos - hSin;
    const y3 = dy + wSin + hCos;
    // Corner 4: -halfW, halfH
    const x4 = dx - wCos - hSin;
    const y4 = dy - wSin + hCos;
