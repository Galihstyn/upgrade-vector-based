...
// ⚡ Bolt: Optimized bounds calculation
// Previously: Math.min(...points.map(...)) allocating 4 arrays and risking stack overflow on large shapes.
// Now: Single O(N) pass, reducing memory allocations and improving performance.
const getPointsBounds = (pts) => {
  let minX = Infinity,
    maxX = -Infinity,
    minY = Infinity,
    maxY = -Infinity;
...
const getElementBounds = (el) => {
  ...
  if (Array.isArray(el.customPoints) && el.customPoints.length > 1) {
    ...
    return {
      ...getPointsBounds(transformedPoints),
    };
  }
