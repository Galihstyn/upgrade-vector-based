...
const getWarpMetrics = (el) => {
  ...
  const positions = chars.map((char, charIdx) => {
    ...
    // Optimize corner calculations to avoid object/array allocations and duplicate trig calls
    const cosA = Math.cos(rad);
    const sinA = Math.sin(rad);

    const x1 = dx + -halfW * cosA - -halfH * sinA;
    const y1 = dy + -halfW * sinA + -halfH * cosA;

    const x2 = dx + halfW * cosA - -halfH * sinA;
    const y2 = dy + halfW * sinA + -halfH * cosA;

    const x3 = dx + halfW * cosA - halfH * sinA;
    const y3 = dy + halfW * sinA + halfH * cosA;

    const x4 = dx + -halfW * cosA - halfH * sinA;
    const y4 = dy + -halfW * sinA + halfH * cosA;
